...
pub fn show(app: &mut CircuitApp, ctx: &Context) {
    ...
    // Central Canvas
    egui::CentralPanel::default().frame(Frame::new().fill(app.theme.panel_color)).show(ctx, |ui| {
        ...
        island_frame.show(ui, |ui| {
            ...
            // Pan & Zoom Logic
            if response.dragged_by(egui::PointerButton::Middle) || response.dragged_by(egui::PointerButton::Primary) && matches!(app.selected_tool, Tool::Select) {
            ...
            // Draw Grid
            crate::ui::drawing::draw_grid(&painter, rect, app.zoom, app.pan, app.theme.dot_color);
            ...
            for comp in &app.state.schematic.components {
                draw_component(
                    ...
                    |p| app.to_screen(p),
                    app.zoom,
                    app.theme.component_body.blend(app.theme.disabled_text_color),
            ...
            if running {
                if let Some(netlist) = &app.active_netlist {
                    for (_coord, &node_id) in &netlist.node_map {
                        // calculate RMS
                        let (sum_sq, count) = app.sim_state.history.iter().rev().take(8000)
            ...
            if let Some(mouse_pos) = response.hover_pos() {
                ...
                match &app.selected_tool.clone() {
                    Tool::PlaceComponent(comp_data) => {
                        crate::ui::tools::placement::handle(app, ui, ctx, &response, &painter, comp_data, grid_pos, snap_pos);
                    },
                    Tool::PlaceWire(start_node) => {
                        crate::ui::tools::wiring::handle(app, ui, ctx, &response, &painter, start_node, grid_pos, snap_pos);
                    },

                    Tool::Select => {
                        crate::ui::tools::selection::handle(app, ui, ctx, &response, &painter, mouse_pos);
                    }

                    Tool::Erase => {
                        crate::ui::tools::erasing::handle(app, ui, ctx, &response, &painter, mouse_pos, grid_pos, snap_pos);
                    }
                    _ => {}
                }
            }
