...
impl<T: CircuitScalar> Triode<T> {
    pub fn new(node_p: NodeId, node_g: NodeId, node_c: NodeId, mu: T, ex: T, kg1: T, kp: T, kvb: T, rgi: T, cgp: T, cgk: T, cpk: T, i_s: T, vt: T) -> Self {
        Self {
            node_p, node_g, node_c,
            cached_idx_p: None, cached_idx_g: None, cached_idx_c: None,
            aux_start_index: None,
            mu, ex, kg1, kp, kvb, rgi, cgp, cgk, cpk, i_s, vt,
            g_min: T::from(1e-9).unwrap(),
            iter_state: Cell::new(TriodeIterationState {
                is_converged: false,
                prev_v_p: T::zero(), prev_v_g: T::zero(),
                prev_v_c: T::zero(), prev_v_x: T::zero(),
            }),
        }
    }
    ...
    /// Helper to stamp a 2-terminal conductance into the admittance matrix
    fn stamp_conductance(matrix: &mut MatMut<T>, n1: Option<usize>, n2: Option<usize>, g: T, offset: usize, a_offset: usize) {
        // checks if we should apply the offset for the auxiliary node (X) instead of the external nodes (P,G,C) and applies it accordingly
        let transform = |n: usize, a: bool| {
            if a {
                n - a_offset
            } else {
                n - offset
        ...
        if let Some(i) = n1 { matrix[(transform(i, true), transform(i, true))] += g; }
        if let Some(j) = n2 { matrix[(transform(j, false), transform(j, false))] += g; }
        if let (Some(i), Some(j)) = (n1, n2) {
            matrix[(transform(i, true), transform(j, false))] -= g;
            matrix[(transform(j, false), transform(i, true))] -= g;
        }
    }

    /// Helper to stamp a transconductance (V_control affecting I_output)
    fn stamp_transconductance(
        matrix: &mut MatMut<T>,
        n_out_pos: Option<usize>, n_out_neg: Option<usize>,
        n_ctrl_pos: Option<usize>, n_ctrl_neg: Option<usize>,
        gm: T,
        offset: usize,
    ) {
        let t = |n: usize| {
        ...
        if let (Some(i), Some(j)) = (n_out_pos, n_ctrl_pos) { matrix[(t(i), t(j))] += gm; }
        if let (Some(i), Some(j)) = (n_out_pos, n_ctrl_neg) { matrix[(t(i), t(j))] -= gm; }
        if let (Some(i), Some(j)) = (n_out_neg, n_ctrl_pos) { matrix[(t(i), t(j))] -= gm; }
        if let (Some(i), Some(j)) = (n_out_neg, n_ctrl_neg) { matrix[(t(i), t(j))] += gm; }
    }

    /// Helper to fetch voltage, defaulting to 0.0 if grounded
    fn get_v(voltages: &ColRef<T>, idx: Option<usize>) -> T {
...
impl<T: CircuitScalar> Component<T> for Triode<T> {
    ...
    fn bake_indices(&mut self, _ctx: &SimulationContext<T>, node_map: &HashMap<NodeId, usize>) {
        self.cached_idx_p = if self.node_p.0 == 0 { None } else { Some(node_map[&self.node_p]) };
        self.cached_idx_g = if self.node_g.0 == 0 { None } else { Some(node_map[&self.node_g]) };
        self.cached_idx_c = if self.node_c.0 == 0 { None } else { Some(node_map[&self.node_c]) };
    }

    fn auxiliary_row_count(&self) -> usize {
        1 // 1 virtual node (X) for the grid diode isolation
    }
    ...
    fn stamp_static(&self, matrix: &mut MatMut<T>, _ctx: &SimulationContext<T>) {
        ...
        Self::stamp_conductance(matrix, self.cached_idx_g, idx_x, g_rgi, 0, 0); // no offset in static necessary

        // Convergence Resistor (between P and C)
        Self::stamp_conductance(matrix, self.cached_idx_p, self.cached_idx_c, self.g_min, 0, 0);
    }

    fn stamp_dynamic(&mut self, prev_node_voltages: &ColRef<T>, rhs: &mut ColMut<T>, ctx: &SimulationContext<T>) {
        let dt = ctx.dt; // Assuming your SimulationContext provides the time step
        let idx_p = self.cached_idx_p;
        let idx_g = self.cached_idx_g;
        let idx_c = self.cached_idx_c;

        let prev_v_p = Self::get_v(prev_node_voltages, idx_p);
        let prev_v_g = Self::get_v(prev_node_voltages, idx_g);
        let prev_v_c = Self::get_v(prev_node_voltages, idx_c);

        // Dynamic Equivalent Currents (I_eq = G_c * V_prev)
        let inject_cap_history = |rhs: &mut ColMut<T>, n1: Option<usize>, n2: Option<usize>, c: T, v1: T, v2: T| {
            let g_c = c / dt;
    ...
    fn stamp_nonlinear(&self, current_node_voltages: &ColRef<T>, matrix: &mut MatMut<T>, rhs: &mut ColMut<T>, ctx: &SimulationContext<T>, l_size: usize) {
        ...
        let v_p = Self::get_v(current_node_voltages, idx_p);
        let v_g = Self::get_v(current_node_voltages, idx_g);
        let v_c = Self::get_v(current_node_voltages, idx_c);
        let v_x = Self::get_v(current_node_voltages, idx_x);
        ...
        // Conductances scale with dt and must be re-evaluated if dt changes dynamically
        if !ctx.is_dc_analysis {
            Self::stamp_conductance(matrix, idx_g, idx_p, self.cgp / dt, l_size, l_size);
            Self::stamp_conductance(matrix, idx_g, idx_c, self.cgk / dt, l_size, l_size);
            Self::stamp_conductance(matrix, idx_p, idx_c, self.cpk / dt, l_size, l_size);
        }
        ...
        let exp_u = Self::exp_safe(u);
        let limit = T::from(20.0).unwrap();
        let (ln_term, l_deriv) = if u > limit {
            (u, T::one())
        } else if u < -limit {
            (T::zero(), T::zero())
        } else {
            let exp_u = u.exp();
            ((T::one() + exp_u).ln(), T::one() / (T::one() + (-u).exp()))
        };

        let e1 = (v_pk / self.kp) * ln_term;

        if e1 > T::zero() {
            i_p = (T::from(2.0).unwrap() / self.kg1) * e1.powf(self.ex);
            ...
            let g_base = (T::from(2.0).unwrap() * self.ex / self.kg1) * e1.powf(self.ex - T::one());
            g_p = g_base * de1_dvpk;
            g_m = g_base * de1_dvgk;
        }
        ...
        let (exp_val, exp_deriv) = if x > max_arg {
            let exp_max = max_arg.exp();
            // Function is exp_max * (1 + x - max_arg)
            // Derivative is exactly exp_max
            (exp_max * (T::one() + (x - max_arg)), exp_max)
        } else {
            let e = x.exp();
            (e, e) // Standard derivative
        };
        ...
        // --- Matrix Stamping ---
        // 1. Plate Output Conductance
        Self::stamp_conductance(matrix, idx_p, idx_c, g_p, l_size, l_size);

        // 2. Plate Mutual Conductance
        Self::stamp_transconductance(matrix, idx_p, idx_c, idx_g, idx_c, g_m, l_size);

        // 3. Grid Diode Dynamic Conductance
        Self::stamp_conductance(matrix, idx_x, idx_c, g_d, l_size, l_size);
        ...
        if let Some(i) = idx_p { rhs[t(i)] -= i_eq_p; }
        if let Some(i) = idx_c { rhs[t(i)] += i_eq_p; }

        // Diode Current leaves X, enters C
        if let Some(i) = idx_x { rhs[t(i)] -= i_eq_d; }
        if let Some(i) = idx_c { rhs[t(i)] += i_eq_d; }
        ...
        self.iter_state.set(TriodeIterationState {
            is_converged: false, // Update logic goes in is_converged() based on dV
            prev_v_p: v_p, prev_v_g: v_g, prev_v_c: v_c, prev_v_x: v_x,
            ..state
        });
    }
    ...
    fn is_converged(&self, current_node_voltages: &ColRef<T>) -> bool {
        ...
        let v_p = Self::get_v(current_node_voltages, self.cached_idx_p);
        let v_g = Self::get_v(current_node_voltages, self.cached_idx_g);
        let v_c = Self::get_v(current_node_voltages, self.cached_idx_c);
        let v_x = Self::get_v(current_node_voltages, self.aux_start_index);
    ...
    fn terminal_currents(&self, node_voltages: &ColRef<T>, _ctx: &SimulationContext<T>, out_currents: &mut [T]) {
        ...
        let v_p = Self::get_v(node_voltages, self.cached_idx_p);
        let v_g = Self::get_v(node_voltages, self.cached_idx_g);
        let v_c = Self::get_v(node_voltages, self.cached_idx_c);
        let v_x = Self::get_v(node_voltages, self.aux_start_index);
        ...
        // 3. Calculate Grid Current (I_G)
        // Current flows into G, through R_GI, to Virtual Node X
        let i_rgi = v_gx / self.rgi;
        let total_i_g = i_rgi;

        // 4. Calculate Cathode Current (I_C)
        // Current flows from X to C through the grid diode
        let exp_vxc_vt = Self::exp_safe(v_xc / self.vt);
        let i_d = self.i_s * (exp_vxc_vt - T::one());

        // Cathode collects plate current, gmin current, and diode current
        // Since current is defined as flowing INTO the terminal, these are negative
        let total_i_c = -i_p_internal - i_gmin - i_d;

        // 5. Write to output slice in the order defined by ports() [P, G, C]
