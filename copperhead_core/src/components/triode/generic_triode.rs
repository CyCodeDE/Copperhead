...
// This model is based on the norman koren triode model.
// While it is very generic, it depends on curve fitting and doesn't completely capture the physics.
// If a specific model is available, it is better to use the specific model instead of the generic.
// The main reason for including this model is to have a simple triode model available for experimentation, testing and versatility.

#[derive(Clone, Debug, PartialEq, serde::Serialize, serde::Deserialize)]
pub struct GenericTriodeDef {
    pub model: GenericTriodeModel,
}
...
struct GenericTriodeIterationState<T: CircuitScalar> {
    pub prev_v_p: T,
    pub prev_v_g: T,
    pub prev_v_c: T,
    pub prev_v_x: T,
...
pub enum GenericTriodeModel {
    _12AX7,
}

impl GenericTriodeModel {
...
pub struct GenericTriode<T: CircuitScalar> {
    ...
    // Simulation State
    iter_state: Cell<GenericTriodeIterationState<T>>,
...
impl<T: CircuitScalar> GenericTriode<T> {
    pub fn new(
        node_p: NodeId,
        node_g: NodeId,
        node_c: NodeId,
        ...
        Self {
            ...
            iter_state: Cell::new(GenericTriodeIterationState {
                prev_v_p: T::zero(),
                prev_v_g: T::zero(),
                prev_v_c: T::zero(),
                prev_v_x: T::zero(),
...
impl<T: CircuitScalar> Component<T> for GenericTriode<T> {
    ...
    fn update_state(&mut self, current_node_voltages: &ColRef<T>, ctx: &SimulationContext<T>) {
        ...
        self.iter_state.set(GenericTriodeIterationState {
            prev_v_p: v_p,
            prev_v_g: v_g,
            prev_v_c: v_c,
            prev_v_x: v_x,
